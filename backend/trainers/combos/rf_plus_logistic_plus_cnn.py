...
from trainers import logistic_regression as lr_module
...
def _ensemble_filename():
    ...
    return f"ensemble_rf_plus_logistic_plus_cnn_{ts}.json"
...
def train(X_train, y_train, X_val, y_val, config=None):
    ...
    if task != "classification":
        raise ValueError("rf_plus_logistic_plus_cnn combo supports classification only")
    ...
    if isinstance(rf_out, tuple):
        ...
        raise RuntimeError(
            "Random Forest trainer must return model when return_model=True"
        )

    lr_out = lr_module.train(X_train, y_train, X_val, y_val, cfg)
    if isinstance(lr_out, tuple):
        lr_result, lr_model, lr_meta = lr_out
    else:
        raise RuntimeError(
            "Logistic Regression trainer must return model when return_model=True"
        )
    ...
    if isinstance(cnn_out, tuple):
        ...
        raise RuntimeError("CNN trainer must return model when return_model=True")

    trained_components = [
        ("random_forest", rf_result, rf_model, rf_meta),
        ("logistic_regression", lr_result, lr_model, lr_meta),
    ...
    for name, result, model, meta in trained_components:
        ...
        total_model_size += int(result.get("model_size", 0))
        total_train_time += float(result.get("train_time", 0.0))

    combined_proba = ensemble_utils.combine_probas(aligned_probas)
    combined_pred_indices = np.argmax(combined_proba, axis=1)
    combined_preds = [canonical_classes[i] for i in combined_pred_indices]
    ...
    ensemble_meta = {
        "ensemble_name": "Random Forest + Logistic Regression + CNN",
        "method": "soft_average",
        "components": component_model_paths,
        "metrics": metrics,
        "train_time_sum": total_train_time,
        "model_size_sum": total_model_size,
        "combine_time": 0.0,
    ...
    result = {
        "model_name": "Random Forest + Logistic Regression + CNN",
