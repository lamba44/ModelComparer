...
def _ensemble_filename():
    ...
    return f"ensemble_rf_plus_mlp_plus_cnn_{ts}.json"
...
def train(X_train, y_train, X_val, y_val, config=None):
    ...
    if task != "classification":
        raise ValueError("rf_plus_mlp_plus_cnn combo supports classification only")
    ...
    combine_time = 0.0  # tiny, could measure if desired

    ensemble_meta = {
        "ensemble_name": "Random Forest + MLP + CNN",
        "method": "soft_average",
    ...
    result = {
        "model_name": "Random Forest + MLP + CNN",
