# trainers/combos/rf_plus_mlp.py
...
def _ensemble_filename():
    ...
    return f"ensemble_rf_plus_mlp_{ts}.json"


def train(X_train, y_train, X_val, y_val, config=None):
    """
    Train Random Forest and MLP on (X_train, y_train) and build a soft-average ensemble
    evaluated on (X_val, y_val). Mirrors the pattern used in rf_plus_logistic.py.
    """
    ...
    if task != "classification":
        raise ValueError("rf_plus_mlp combo supports classification only")
    ...
    combine_time = 0.0  # negligible here; could time if desired

    ensemble_meta = {
        "ensemble_name": "Random Forest + MLP",
        "method": "soft_average",
