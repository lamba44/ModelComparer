...
def _ensemble_filename():
    ...
    return f"ensemble_rf_plus_logistic_plus_mlp_{ts}.json"
...
def train(X_train, y_train, X_val, y_val, config=None):
    ...
    if task != "classification":
        raise ValueError("rf_plus_logistic_plus_mlp combo supports classification only")
    ...
    if isinstance(lr_out, tuple):
        ...
        raise RuntimeError(
            "Logistic regression trainer must return model object when return_model=True"
        )
    ...
    ensemble_meta = {
        "ensemble_name": "Random Forest + Logistic Regression + MLP",
        "method": "soft_average",
    ...
    result = {
        "model_name": "Random Forest + Logistic Regression + MLP",
