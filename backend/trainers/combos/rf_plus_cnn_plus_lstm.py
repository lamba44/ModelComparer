...
def _ensemble_filename():
    ...
    return f"ensemble_rf_plus_cnn_plus_lstm_{ts}.json"
...
def train(X_train, y_train, X_val, y_val, config=None):
    ...
    if task != "classification":
        raise ValueError("rf_plus_cnn_plus_lstm combo supports classification only")
    ...
    combine_time = 0.0  # small; measure if needed

    ensemble_meta = {
        "ensemble_name": "Random Forest + CNN + LSTM",
        "method": "soft_average",
    ...
    result = {
        "model_name": "Random Forest + CNN + LSTM",
