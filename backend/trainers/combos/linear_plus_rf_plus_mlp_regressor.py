...
from trainers import random_forest_regressor as rf_module
from trainers import mlp_regressor as mlp_module
...
def _ensemble_filename():
    ...
    return f"ensemble_linear_plus_rf_plus_mlp_regressor_{ts}.json"
...
def train(X_train, y_train, X_val, y_val, config=None):
    ...
    if task != "regression":
        raise ValueError("This combo supports regression only")
    ...
    if isinstance(mlp_out, tuple):
        ...
        raise RuntimeError(
            "MLP regressor trainer must return model object when return_model=True"
        )

    trained_components = [
        ("linear_regression", lin_result, lin_model, lin_meta),
        ("random_forest_regressor", rf_result, rf_model, rf_meta),
        ("mlp_regressor", mlp_result, mlp_model, mlp_meta),
    ]
    ...
    for name, result, model, meta in trained_components:
        preds = _safe_predict(model, X_val)
    ...
    stacked = np.vstack(preds_list)  # shape: (n_models, n_samples)
    combined_pred = np.mean(stacked, axis=0)

    metrics = metrics_module.compute_metrics(y_val, combined_pred, task="regression")

    ensemble_meta = {
        "ensemble_name": "Linear Regression + Random Forest Regressor + MLP Regressor",
        "method": "mean_average",
    ...
    result = {
        "model_name": "Linear Regression + Random Forest Regressor + MLP Regressor",
