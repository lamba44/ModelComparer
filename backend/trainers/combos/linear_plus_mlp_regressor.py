...
from trainers import mlp_regressor as mlp_reg_module
...
def _ensemble_filename():
    ...
    return f"ensemble_linear_plus_mlp_regressor_{ts}.json"
...
def train(X_train, y_train, X_val, y_val, config=None):
    ...
    if task != "regression":
        raise ValueError("linear_plus_mlp_regressor supports regression only")
    ...
    if isinstance(lin_out, tuple):
        ...
        raise RuntimeError(
            "Linear regression trainer must return model when return_model=True"
        )

    mlp_out = mlp_reg_module.train(X_train, y_train, X_val, y_val, cfg)
    ...
    ensemble_meta = {
        "ensemble_name": "Linear Regression + MLP Regressor",
        "method": "mean_average",
    ...
    return {
        "model_name": "Linear Regression + MLP Regressor",
